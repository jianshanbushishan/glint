...
“常规”集中设置触发键、主题、开机启动、轨迹样式和日志级别，提供轨迹预览。常规页可直接打开日志文件，高级选项提供配置目录入口。版本号显示在左下角。

“全局手势”和各应用标签使用同一套手势列表与动作编辑区，可选择或记录轨迹、录入并确认快捷键、配置窗口操作或启动程序。添加应用时，可选择程序或拾取窗口取得完整 `.exe` 路径；路径右侧的“更改应用程序”可替换程序并保留手势配置。应用启用后，可选择未单独配置的手势“使用全局配置”或“不执行”；停用会保留配置、隐藏策略及编辑区，并在捕获手势前放行鼠标输入。标题右侧的删除图标需确认后才会移除应用配置，随后恢复全局手势。

点击“拾取窗口”后，设置界面暂时隐藏，鼠标变为十字光标；左键点击目标窗口即可恢复设置界面并填入应用名称和程序路径。按 Esc 或等待 60 秒可取消拾取，保留原有输入。

右下角“应用设置”保存当前页面或当前应用作用域的修改。切换标签会保留各自草稿，其他标签不会随当前作用域一起提交；关闭窗口时若仍有未应用修改，会提示是否放弃。主题选择和开机启动开关自动保存。也可从界面打开配置目录编辑 JSON 基础配置。
